...
def kspace_filter_fast(map, vk_mask=None, hk_mask=None, normalize="phys", inv_pixwin_lxly=None):
...
def get_filtered_map(orig_map, binary, vk_mask, hk_mask, normalize="phys", inv_pixwin_lxly=None):
    ...
    if orig_map.ncomp == 1:
        ...
        orig_map.data[:] *= binary.data
    filtered_map = kspace_filter_fast(
        orig_map, vk_mask, hk_mask, normalize=normalize, inv_pixwin_lxly=inv_pixwin_lxly
    )
            ...
            for id_ar1, ar1 in enumerate(arrays_1):
                for id_sv2, sv2 in enumerate(surveys):
                    ...
                    for id_ar2, ar2 in enumerate(arrays_2):
                        ...
                        if id_sv1 > id_sv2:
                            continue
                        if (sv1 != sv2) & (kind == "noise"):
                            continue
                        if (sv1 != sv2) & (kind == "auto"):
                            continue
                        nspec[kind] += 1
...
def get_noise_matrix_spin0and2(noise_dir, survey, arrays, lmax, nsplits):
    ...
    for c1, ar1 in enumerate(arrays):
        for c2, ar2 in enumerate(arrays):
            ...
            l, nl = so_spectra.read_ps(
                "%s/mean_%sx%s_%s_noise.dat" % (noise_dir, ar1, ar2, survey), spectra=spectra
            )
            nl_t = nl["TT"][:lmax]
...
def remove_mean(somap, window):
    ...
    if somap.ncomp == 1:
        somap.data -= np.mean(somap.data * window.data)
    else:
        somap.data[0] -= np.mean(somap.data[0] * window[0].data)
        somap.data[1] -= np.mean(somap.data[1] * window[1].data)
        somap.data[2] -= np.mean(somap.data[2] * window[1].data)

    return somap


def deconvolve_tf(lb, ps, tf1, tf2, lmax=None):
...
def fast_cov_coupling(
    sq_win_alms_dir, na_r, nb_r, nc_r, nd_r, lmax, l_exact=None, l_band=None, l_toep=None
):
    ...
    cov_fortran.calc_cov_spin0(
        wcl["TaTcTbTd"], wcl["TaTdTbTc"], l_exact, l_band, l_toep, coupling.T
    )
...
def covariance_element(
    coupling, id_element, ns, ps_all, nl_all, binning_file, mbb_inv_ab, mbb_inv_cd
):
...
def g(a, b, c, d, ns):
    ...
    result = 1.0 * ns[a] * (ns[c] * delta2(a, b) * delta2(c, d) - delta4(a, b, c, d))
